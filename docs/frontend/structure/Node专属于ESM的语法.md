...
• `import.meta.url`返回当前模块的完整文件 URL（`file://` 协议）。  console.log(import.meta.url); // 输出: file:///path/to/module.mjs
• `import.meta.resolve()`（实验性）解析模块路径（类似 `require.resolve`），返回 Promise。  const resolvedPath = await import.meta.resolve('lodash');
